  ...
  block:
    ...
    block:
      if ((posc1 == posc2) or (posc1 == posc3) or (posc1 == posc4)) or ((posc2 == slot) or (posc3 == slot) or (posc4 == slot)):
        ...
        block:
            posc1 := slot 
            sirkelListe = [list: pawn]   
            posisjonsListe = [list: slot] 
            moveListe = [list: movenum]
            cirlog := cirlog.append(sirkelListe)    
            poslog := poslog.append(posisjonsListe)
            record := record.append(moveListe)
            movenum := movenum + 1
        end
      end
    end
